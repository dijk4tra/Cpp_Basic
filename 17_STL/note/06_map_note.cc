...
void print(Container &box)
{
     ...
     *
     * 注意：
     * map 中的 key 是 const 的，
     * 不能通过迭代器直接修改 key。
     *
     * 原因：
     * map 会根据 key 维护内部排序结构，
     * 如果允许直接修改 key，
     * 可能会破坏 map 的有序结构。
     */
    for(auto & p : box)
...
void test3()
{
    // 创建一个空的 map<string, int> 容器
    map<string,int> box;

    /**
     * 插入单个键值对
     *
     * map 中保存的是 key-value 键值对，
...
void test6()
{
     ...
     *
     * 创建 map 时，
     * map 会使用 StudentCompare 中的 operator()
     * 对 Student 类型的 key 进行排序和去重。
     */
